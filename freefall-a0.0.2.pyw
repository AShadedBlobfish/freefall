...
note = tk.Label(master=topContainer, text="10 pixels = 1 metre")
label.pack(side=tk.LEFT, padx=(5,0))
entry.pack(side=tk.LEFT)
unit.pack(side=tk.LEFT)

heightLabel = tk.Label(master=topContainer, text="h =")
f = tk.StringVar(value="10", name="h")
global heightEntry
...
def start():
    ...
    k.set("v = ")
    turtle.speed(5)
    turtle.sety((float(heightEntry.getvar("h"))*10)-210)
    turtle.showturtle()
    turtle.speed(0)
    ...
    while turtle.pos()[1] > -210:
        start = time.time()
        yvel += float(entry.getvar("g"))*0.25
        y -= (yvel*0.25)/10
        turtle.sety(y)
        time.sleep((0.25 - (time.time() - start))/20)
